...
if ($plan_type == "members") {
    ...
    if(!empty($payment_data_query)) {
        ...
        foreach ($payment_data_query as $key => $table_data) {
            ...
            $table_content_html .= "<tr class='arm_member_last_subscriptions_data'>";
                ...
                $table_content_html .= "<td>".number_format($table_data['arm_amount'],$arm_currency_decimal)." ".$table_data['arm_currency']."</td>"; 
            ...
            if (isset($payment_gt_data[$i]) && !empty($payment_gt_data[$i])) {
                ...
                foreach ($payment_gt_data[$i] as $key => $value) {
                    if ($value['name'] == "manual") {
                        $name .= __("Paid By Admin", "ARMember")." : ".number_format($value['total'],$arm_currency_decimal)."<br>";
                    } else {
                        $title = ucwords(str_replace("_", " ", $value['name']));
                        $name .= "{$title} : ".number_format($value['total'],$arm_currency_decimal,'.','')."<br>";
                    }
                    $amount += $value['total'];
                }
                $tool_tip .= "'{$name}', ";
        ...
        for ($i = 1; $i <= $day_last; $i++) {
            ...
            if (isset($payment_gt_data[$i]) && !empty($payment_gt_data[$i])) {
                ...
                foreach ($payment_gt_data[$i] as $key => $value) {
                    ...
                    $amount += number_format($value['total'],$arm_currency_decimal,'.','');
                }
                $tool_tip .= "'{$name}', ";
            } else {
                $tool_tip .= "' ', ";
            ...
            if (isset($payment_gt_data[$current_hour]) && !empty($payment_gt_data[$current_hour])) {
                foreach ($payment_gt_data[$current_hour] as $key => $value) {
                    ...
                    if (!empty($name) && strpos($name, $value['name']) === false) {
                        $total = number_format( $value['total'], $arm_currency_decimal,'.','' );
                        $name .= "{$value['name']} : ".number_format($value['total'],$arm_currency_decimal,'.','')."<br>";
                    } else {
                        $total = number_format( $previous_amt, $arm_currency_decimal,'.','' ) + number_format( $value['total'], $arm_currency_decimal,'.','' );
    ...
    <?php if(isset($_REQUEST['action']) && $_REQUEST['action']=="armupdatecharts"){?>
        ...
        <div class="armchart_plan_section">
            <?php if(isset($arm_charts_plan_data) && count($arm_charts_plan_data)>0){
                ...
                <div class="armchart_plan_item">
                    <label class="armchart_plan_title"><?php echo $arm_charts_plan_row['plan_name']; ?></label>
                    <p class="armchart_plan_item_desc"><?php echo __("Total Transactions:", "ARMember");?> <?php echo count($arm_charts_plan_row['plan_transaction_user_ids']); ?><span class="arm_chart_total_amount"><?php echo __("Total Amount:", "ARMember");?> <?php echo number_format($arm_charts_plan_row['total_amount'], $arm_currency_decimal); ?></p>
                </div>
            <?php 
                }
            }else{
    ...
    if(!empty($payment_data_query)) {
        ...
        foreach ($payment_data_query as $key => $table_data) {
            ...
            $table_content_html .= "<td>".number_format($arm_coupon_discount,$arm_currency_decimal)." ".$arm_coupon_discount_type."</td>";
            $table_content_html .= "<td>".$paid_by."</td>"; 
            $table_content_html .= "<td>".$arm_plan_name."</td>"; 
            $table_content_html .= "<td>".number_format($table_data['arm_amount'],$arm_currency_decimal)." ".$table_data['arm_currency']."</td>"; 
            $table_content_html .= "<td>".$payment_gateway."</td>";   
    ...
    <?php if(isset($_REQUEST['action']) && $_REQUEST['action']=="armupdatecharts"){?>
        ...
        ?>
                <div class="armchart_plan_item">
                    ...
                    <p class="armchart_plan_item_desc"><?php echo __("Coupon Code Used:", "ARMember");?> <?php echo count($arm_charts_plan_row['coupon_users']); ?><span class="arm_chart_total_amount"><?php echo __("Total Amount:", "ARMember");?> <?php echo number_format($arm_charts_plan_row['total_amount'], $arm_currency_decimal); ?></p>
                </div>
        <?php 
                }
            }else {
