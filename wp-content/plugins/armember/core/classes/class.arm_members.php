                ...
                if (is_array($ids)) {
                    if (!empty($file_data_array)) {
                        ...
                        if (!empty($users_data)) {
                            ...
                            if (!empty($planObj) && !$planObj->is_free() && $planobj->type!='paid_infinite') {
            ...
            if ($sSearch != '') 
            {
                ...
                if(!empty($filter_meta_field_key)){
                    ...
                    if(in_array($filter_meta_field_key, $arm_user_keys)){
                        ...
                        $arm_search_user_meta_key_where .= " (um.meta_key = '".$filter_meta_field_key."' AND um.meta_value LIKE '%{$sSearch}%')  ";
                    }
            ...
            if($filter_status_id != '') {
                if(strpos($filter_status_id, "5") !== false && strpos($filter_status_id, "5") == 0) {
                    ...
                    if(strpos($filter_status_id, "5") !== false) {
                        ...
                        $join_for_status = " LEFT JOIN `{$usermeta_table}` um1 ON um1.user_id = u.ID ";
                        $search_where .= " (armu.arm_user_suspended_plan_ids != 'a:0:{}' AND armu.arm_user_suspended_plan_ids != '')";
                    }
                    if(strpos($filter_status_id, "6") !== false) {

                        $is_suspended = false;
                        $filter_status_id = substr($filter_status_id, 0, -2);
                        $search_where .= "( (armu.arm_user_plan_ids != 'a:0:{}' AND armu.arm_user_plan_ids IS NOT NULL AND armu.arm_user_plan_ids !='') AND (armu.arm_user_suspended_plan_ids = '' OR armu.arm_user_suspended_plan_ids = 'a:0:{}' or armu.arm_user_suspended_plan_ids IS NULL) ) ";
                    }
