...
if (!class_exists('ARM_payment_gateways')) {
    ...
    class ARM_payment_gateways {
        ...
        function arm_view_general_debug_log()
        {
            ...
            if(!empty($_POST) && !empty($_POST['arm_debug_log_selector']))
            {
                ...
                if($totalRecord>0)
                {
                    ...
                    $arm_general_debug_log_data = $wpdb->get_results("SELECT * FROM `" . $tbl_arm_debug_general_log . "` WHERE {$arm_general_debug_log_selector_search} ORDER BY arm_general_log_id DESC {$armgeneralDebugLimit}", ARRAY_A);
                        ...
                        foreach($subscription_plan as $pid){
                             ...
                             * */
                                ...
                                if ($update) {
                                    ...
                                    $arm_subscription_recurring_time = !empty($arm_meta_plan_options['payment_cycles'][$payment_cycle]) ? $arm_meta_plan_options['payment_cycles'][$payment_cycle]['recurring_time'] : '';
        ...
        function arm_bank_transfer_payment_gateway_action($payment_gateway, $payment_gateway_options, $posted_data, $entry_id = 0) {
            global $wpdb, $ARMember, $arm_global_settings, $payment_done, $arm_membership_setup, $arm_manage_coupons, $arm_subscription_plans, $arm_transaction, $arm_debug_payment_log_id,$arm_manage_communication;
