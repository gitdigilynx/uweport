...
', 'ARMember');?> 
                ...
                ?>
                <tr class="form-field" id="changeCurrency">
					<th class="arm-form-table-label"><?php _e('Number of decimals','ARMember');?></th>
					<td class="arm-form-table-content">
                    <?php $general_settings['arm_currency_decimal_digit'] = (isset($general_settings['arm_currency_decimal_digit']) && $general_settings['arm_currency_decimal_digit']!= '') ? $general_settings['arm_currency_decimal_digit'] : 2; ?>
						<input type='hidden' id='arm_currency_decimal_digit' name="arm_general_settings[arm_currency_decimal_digit]" value="<?php echo $general_settings['arm_currency_decimal_digit'];?>" />
						<dl class="arm_selectbox column_level_dd" id="arm_currency_decimal">
							<dt><span class="arm_no_auto_complete"><?php echo $general_settings['arm_currency_decimal_digit'];?></span><i class="armfa armfa-caret-down armfa-lg"></i></dt>
							<dd>
                    ...
                    <td class="popup_content_text arm_clear_field_wrapper">
                        ...
                        if (!empty($dbProfileFields['other'])) {
                            ...
                            foreach ($dbProfileFields['other'] as $fieldMetaKey => $fieldOpt) {
                                ...
                                $fchecked = '';
                                if ($wpdb->get_var("SELECT count(`arm_form_field_slug`) FROM `" . $ARMember->tbl_arm_form_field . "` WHERE `arm_form_field_slug`='" . $fieldMetaKey . "' AND arm_form_field_form_id!='0'") > 0) {
