...
if (!class_exists('ARM_member_forms')) {
                        ...
                        if (!empty($planData)) {
                            ...
                            <td class="arm-form-table-content">';
                                    ...
                                    if (isset($is_plan_cancelled) && $is_plan_cancelled == 'yes') {
                                        ...
                                        $success_msg = __('Your Subscription will be canceled on', 'ARMember') . ' ' . $expire_time . ".";
                                        $content .= $success_msg;
                                    } else {
    ...
    margin-left: 10px;position: absolute;"></div>';
                        ...
                        foreach (array('facebook', 'twitter', 'linkedin', 'vk', 'insta','tumblr') as $social_type) {
                            ...
                            if($social_type == "insta") {
                                if(isset($_REQUEST["display_name"]) && !empty($_REQUEST["display_name"])) {
                                    $form_content .= '<input type="hidden" name="display_name" value="' . sanitize_text_field($_REQUEST["display_name"]) . '"/>';
                                }
                                else if(isset($_REQUEST["full_name"]) && !empty($_REQUEST["full_name"])) {
                                    $form_content .= '<input type="hidden" name="display_name" value="' . sanitize_text_field($_REQUEST["full_name"]) . '"/>';
                                }
                            }
                                    ...
                                    ?>
                                                ...
                                                <?php _e('File Size Limit', 'ARMember'); ?>
                                                    ...
                                                    <div class="arm_form_field_settings_field_val">
                                                        ...
                                                        echo __('PHP Maximum Upload Size:', 'ARMember') . ' '. $upload_mb . __('MB', 'ARMember');
                                                        ?>
                                                        </p>
                                                    </div>
                                                </div>
        ...
        function arm_member_form_get_fields_by_type($field_options, $field_id = 0, $form_id = 0, $form_type = 'inactive', $form = '', $formRandomID = '') {
            ...
            $isEditProfile = ($form->type == 'edit_profile') ? 1 : 0;
            ...
            foreach($all_fields as $fields)
            {
                if(!empty($fields['arm_form_field_option']['meta_key']) && $fields['arm_form_field_option']['meta_key'] == $name)
                {
                    ...
                    if (!empty($form_field_option['options']['minlength'])) {
                        ...
                        $minlength_invalid_message = (isset($common_messages['arm_minlength_invalid']) && $common_messages['arm_minlength_invalid'] != '') ? str_replace('[MINVALUE]', ((int) $form_field_option['options']['minlength']), $common_messages['arm_minlength_invalid']) : __('Please enter at least', 'ARMember') . " " . ((int) $form_field_option['options']['minlength']) .' '. __('characters.', 'ARMember');
                    ...
                    if (!empty($form_field_option['options']['maxlength'])) {
                        ...
                        $maxlength_invalid_message = (isset($common_messages['arm_maxlength_invalid']) && $common_messages['arm_maxlength_invalid'] != '') ? str_replace('[MAXVALUE]', ((int) $form_field_option['options']['maxlength']), $common_messages['arm_maxlength_invalid']) : __('Maximum', 'ARMember') . " " . ((int) $form_field_option['options']['minlength']) .' '. __('characters allowed.', 'ARMember');
                ...
                case 'profile_cover':
                    ...
                    if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array($arm_slugs->manage_members))) {
                        ...
                        if ($display_file) {
                            ...
                            $output .= '<div class="arm_old_uploaded_file arm_admin_file"><a href="' . $field_val . '" target="__blank">'.__('View File', 'ARMember').'</a></div>';
                        }
                ...
                case "radio":
                    ...
                    if (!empty($value['options'])) {
                        if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array($arm_slugs->manage_members))) {
                            ...
                            if(is_admin() && ((!empty($_REQUEST['page']) && $_REQUEST['page']=='arm_manage_forms') || (!empty($_REQUEST['action']) && $_REQUEST['action']=='arm_create_new_field') || !$ARMember->arm_check_is_elementor_page()) ) {
                    ...
                    if (!empty($value['options'])) {
                        if (is_admin() && isset($_GET['page']) && in_array($_GET['page'], array($arm_slugs->manage_members))) {
                            ...
                            foreach ($value['options'] as $data) {
                                ...
                                $output .= '<input type="hidden" name="' . esc_attr($fhname) . '" id="' . esc_attr($value['id']) . '_' . $arm_form_chkbox_counter . '_' . $form_id . '_arm_hidden" value="' . $hidden_key . '">';

                                $output .= '<input class="arm_icheckbox arm_hidden_checkbox ' . esc_attr($class) . '" type="checkbox" name="' . esc_attr($fname) . '" id="' . esc_attr($value['id']) . '_' . $arm_form_chkbox_counter . '_' . $form_id . '" value="' . esc_attr($key) . '"  data-id="' . esc_attr($value['id']) . '_' . $arm_form_chkbox_counter . '_' . $form_id . '_arm_hidden" ' . $chked . ' ' . $validation_data . '/>';
                                $output .= '<label class="arm_checkbox_label" for="' . esc_attr($value['id']) . '_' . $arm_form_chkbox_counter . '_' . $form_id . '">' . $option . '</label>';
                ...
                if (!$validate) {
                    ...
                    if (!empty($armform->fields)) {
                        foreach ($armform->fields as $field) {
                            ...
                            if (isset($posted_data[$field_name]) && isset($form_field_option['required']) && $form_field_option['required'] == 1 && ( $posted_data['form_type'] == 'edit_profile' && empty($form_field_option['readonly']))) {
                            ...
                            if (($form_field_option['type'] == 'file' || $form_field_option['type'] == 'avatar' || 'profile_cover' == $form_field_option['type']) && ( $posted_data['form_type'] == 'edit_profile' && empty($form_field_option['readonly'] ) ) ){
        ...
        function arm_prefix_suffix_field_html() {
            ...
            $iconColor = isset( $_POST['color'] ) ? sanitize_text_field( $_POST['color'] ) : '';
			$icon_key  = isset( $_POST['iconkey'] ) ? sanitize_text_field( $_POST['iconkey'] ) : '';
			$icon = isset( $_POST['icon'] ) ? sanitize_text_field( $_POST['icon'] ) : '';
			$field_id = intval( $_POST['field_id'] );
            $type = sanitize_text_field( $_POST['type'] );
            $icon =$icon_key.' '. $icon;
            if (!empty($icon)) {
                echo $this->arm_generate_field_fa_icon($field_id, $icon, $type, $iconColor);
            } else {
                echo '';
            }
            exit;
        ...
        {
            $session_arr = array();
            $form_key_arr = $_POST['form_key_arr'];
