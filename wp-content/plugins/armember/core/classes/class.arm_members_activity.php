...
if (!class_exists('ARM_members_activity'))
{
	class ARM_members_activity
	{
		...
		if (version_compare($arm_newdbversion, '5.9.3', '<') && version_compare($arm_newdbversion,'1.8.1','>')) {
	...
	function generateplugincode() {
        ...
        function arm_crop_image() {
			...
			$update_meta = isset( $_POST['update_meta'] ) ? sanitize_text_field( $_POST['update_meta'] ) : '';
			$type = isset( $_POST['type'] ) ? sanitize_text_field( $_POST['type']) : '';
			$cord = isset( $_POST['cord'] ) ? sanitize_text_field( $_POST['cord']) : '';
			$user_id = get_current_user_id();

			$arm_src = isset( $_POST['src'] ) ? esc_url_raw( $_POST['src'] ) : '';
			$arm_rotate = isset( $_POST['rotate'] ) ? sanitize_text_field($_POST['rotate']) : '';

			/*this change need to confirm with multisite*/
			$arm_src = MEMBERSHIP_UPLOAD_URL . '/' . basename( $arm_src );

			$info     = getimagesize( MEMBERSHIP_UPLOAD_DIR . '/' . basename( $arm_src ) );
			$file     = esc_url_raw( $arm_src );
			$file1    = MEMBERSHIP_UPLOAD_DIR . '/' . basename( $arm_src );
