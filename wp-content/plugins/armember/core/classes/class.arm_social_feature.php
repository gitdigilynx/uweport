    ...
    class ARM_social_feature {
        ...
        function arm_activate_plugin() {
            ...
            if (( $network_wide && !isset($current[$plugin]) ) || (!$network_wide && !in_array($plugin, $current) )) {
                ...
                include_once( WP_PLUGIN_DIR . '/' . $plugin ); //phpcs:ignore
                    ...
                    if (!empty($action_type)) {
                        ...
                        if($action_type=='facebook' )
                        {
                            ...
                            if(!empty($get_fb_user_details['body']) && !is_wp_error($get_fb_user_details))
                            {
                                ...
                                $posted_data['first_name'] = isset($get_fb_user_details['first_name']) ? $get_fb_user_details['first_name'] : '';
                                $posted_data['last_name'] = isset($get_fb_user_details['last_name']) ? $get_fb_user_details['last_name'] : '';
                                $posted_data['birthday'] = isset($get_fb_user_details['birthday']) ? $get_fb_user_details['birthday'] : '';
                                $posted_data['user_profile_picture'] = $posted_data['picture'] = isset($get_fb_user_details['picture']['data']['url']) ? $get_fb_user_details['picture']['data']['url'] : '';
                                $posted_data['gender'] = isset($get_fb_user_details['gender']) ? $get_fb_user_details['gender'] : '';
                                $posted_data['display_name'] = isset($get_fb_user_details['display_name']) ? $get_fb_user_details['display_name'] : $posted_data['first_name']." ".$posted_data['last_name'];
                            }
                        }
                        ...
                        $social_id = isset($posted_data['userId']) ? $posted_data['userId'] : '';
                        $user_data = array(
        ...
        function arm_login_with_twitter() {
            ...
            if (isset($_GET['page']) && in_array($_GET['page'], array('arm_login_with_twitter')) && !empty($arm_social_feature->isSocialLoginFeature) ) {
                ...
                require_once (MEMBERSHIP_LIBRARY_DIR . '/twitter/OAuth.php');
		...
		require_once (MEMBERSHIP_LIBRARY_DIR . '/twitter/OAuth.php');
                require_once (MEMBERSHIP_LIBRARY_DIR . '/twitter/twitteroauth.php');
                $_SESSION['arm_tw_oauth_token'] = isset($_SESSION['arm_tw_oauth_token']) ? $_SESSION['arm_tw_oauth_token'] : $_REQUEST['oauth_token'];
		$oauth_verifier = isset($_GET['oauth_verifier']) ? $_GET['oauth_verifier'] : '';
                $_SESSION['arm_tw_oauth_token_secret'] = isset($_SESSION['arm_tw_oauth_token_secret']) ? $_SESSION['arm_tw_oauth_token_secret'] : $oauth_verifier;
                ...
                $connection = new TwitterOAuth($tw_conf['customer_key'], $tw_conf['customer_secret'], $access_token['oauth_token'], $access_token['oauth_token_secret']);
