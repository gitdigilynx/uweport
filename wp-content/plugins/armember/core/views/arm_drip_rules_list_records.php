				...
				<table cellpadding="0" cellspacing="0" border="0" class="display arm_hide_datatable" id="armember_datatable">
					<thead>
						<tr>
							...
							<th class="arm_width_80 center"><?php _e('Enable','ARMember');?></th>
							<th class="arm_width_120"><?php _e('Content Type','ARMember');?></th>
							<th class="arm_width_250"><?php _e('Page/Post Name','ARMember');?></th>
							<th class="arm_width_400"><?php _e('Drip Type', 'ARMember'); ?></th>
							<th class="arm_width_200"><?php _e('Shortcode','ARMember');?></th>
							<th class="arm_width_300"><?php _e('Plans','ARMember');?></th>
						...
						foreach ($all_drip_rules as $dr) {
							...
							<tr class="arm_drip_rules_tr_<?php echo $ruleID;?> row_<?php echo $ruleID;?>">
								...
								switch ($rule_type) {
									case 'instant':
										...
										if(!empty($dr['rule_options']['rule_expire_immediate']))
										{
											...
											{
												$expire_duration = __('Day(s)', 'ARMember');
											}
											else if($expire_duration == 'month')
											{
												$expire_duration = __('Month(s)', 'ARMember');
											}
											else
											{
												$expire_duration	 = __('Year(s)', 'ARMember');
											}
											$expire_duration_time = isset($dr['rule_options']['expire_duration_immediate_time']) ? $dr['rule_options']['expire_duration_immediate_time'] : '';
											$rule_type_text .= '<br/>'.__('Hide after', 'ARMember') . ' ' . $exp_days . ' '. $expire_duration .' '. __('of subscription', 'ARMember');
											$rule_type_text .=  (!empty($expire_duration_time)) ? ' '. __('at','ARMember').' '.$expire_duration_time : '';
									...
									case 'days':
										...
										if($duration == 'day')
										{
											$duration = __('Day(s)', 'ARMember');
										}
										else if($duration == 'month')
										{
											$duration = __('Month(s)', 'ARMember');
										}
										else
										{
											$duration = __('Year(s)', 'ARMember');
										}

										$duration_time = isset($dr['rule_options']['duration_time']) ? $dr['rule_options']['duration_time'] : '';
										$rule_type_text = __('Show after', 'ARMember') . ' ' . $days . ' '. $duration .' '. __('of subscription', 'ARMember');
										$rule_type_text .=  (!empty($duration_time)) ? ' '. __('at','ARMember').' '.$duration_time : '';
									...
									case 'post_publish':
										...
										$rule_type_text = __('Show after', 'ARMember') . ' ' . $post_publish . ' '. $duration .' '. __('of post is published', 'ARMember');
										$rule_type_text .=  (!empty($duration_time)) ? ' '. __('at','ARMember').' '.$duration_time : '';
										...
										if(!empty($dr['rule_options']['rule_expire_post_publish']))
										{
											...
											$rule_type_text .= '<br/>'.__('Hide After', 'ARMember') . ' ' . $exp_days . ' '. $expire_duration .' '. __('of post is published', 'ARMember');
											$rule_type_text .=  (!empty($expire_duration_time)) ? ' '. __('at','ARMember').' '.$expire_duration_time : '';
									...
									case 'post_modify':
										...
										$rule_type_text = __('Show after', 'ARMember') . ' ' . $post_modify . ' '. $duration .' '.__('of post is last modified', 'ARMember');
										$rule_type_text .=  (!empty($duration_time)) ? ' '. __('at','ARMember').' '.$duration_time : '';
										...
										if(!empty($dr['rule_options']['rule_expire_post_modify']))
										{
											...
											$rule_type_text .= '<br/>'.__('Hide After', 'ARMember') . ' ' . $exp_days . ' '. $expire_duration .' '. __('of post is last modified', 'ARMember');
											$rule_type_text .=  (!empty($expire_duration_time)) ? ' '. __('at','ARMember').' '.$expire_duration_time : '';
								...
								<td>
									...
									$item_title = "-";
									if($dr['arm_item_type'] == 'custom_content'){
										$shortCode = "[arm_drip_content id='{$ruleID}']".__('Put Your Drip Content Here.', 'ARMember');
										$shortCode .= "[arm_drip_else]";
										$shortCode .= __('Put Your Restricted Content Message Here.', 'ARMember');
