    ...
    class ARM_Stripe {
                ...
                if ($payment_mode == 'auto_debit_subscription') {
                    ...
                    if (!empty($amount)) {
                        if (!in_array($currency, $zero_demial_currencies)) {
                            $amount = $amount * $arm_stripe_amount_multiply;
        ...
        function arm_StripePayment($charge_details, $plan='', $payment_mode='manual_subscription') {
            ...
            if (isset($all_payment_gateways['stripe']) && !empty($all_payment_gateways['stripe'])) {
                ...
                $tax_percentage = isset($charge_details['tax_percentage']) ? number_format((float)$charge_details['tax_percentage'], $arm_currency_decimal, '.',''): 0;
               ...
               try {
                    ...
                    $actual_error = isset($error_msg['error']['message']) ? $error_msg['error']['message'] : '';
                    $arme_msg=(!empty($e->getMessage()))?$e->getMessage():'';
                    $actual_error = !empty($actual_error) ? $actual_error.' '.$arm_help_link : $arme_msg;                    
                                    ...
                                    if (in_array($entry_plan, $userPlan)) {
                                        ...
                                        switch ($event->type) {
                                            ...
                                            case 'customer.subscription.updated':
                                                ...
                                                if( 'customer.subscription.updated' == $event->type && empty($entry_id) ){
                                                    ...
                                                    do_action('arm_payment_log_entry', 'stripe', 'webhook data get customer email', 'payment_gateway', $customer_email, $arm_debug_payment_log_id);
                                                    ...
                                                    do_action('arm_payment_log_entry', 'stripe', 'webhook data get entry_id row from entries', 'payment_gateway', $getEntryId, $arm_debug_payment_log_id);
                                            ...
                                            case 'subscription_schedule.canceled':
                                                ...
                                                //Check Subscription ID in usermeta and then after cancel it.
        ...
        function arm_store_stripe_log($stripe_response = '', $plan_id = 0, $user_id = 0, $payer_email = '', $extraVars = array(), $payment_mode = '', $coupon_details = array()) {
            ...
            if (!empty($stripe_response)) {
                ...
                if (!empty($stripe_response->plan) && $stripe_response->object == 'subscription') {
                    ...
                    $amount = !empty($stripe_response->plan->amount_decimal) ? $stripe_response->plan->amount_decimal : $stripe_response->plan->amount;
                    $currency = strtoupper($stripe_response->plan->currency);
                    if (!in_array($currency, $zero_demial_currencies)) {
                        $amount = $amount / $arm_stripe_amount_multiply; /* amount comes in as amount in cents, so we need to convert to dollars */
                    }
					...
					$arm_payment_date = "";
					if(!empty($stripe_response->current_period_start))
					{
                    ...
                    if( isset($stripe_response->discount) && $stripe_response->discount != null) {
                    ...
                    if((empty($stripe_response->discount) || $stripe_response->discount == null || $stripe_response->discount == 'null') && !empty($coupon_code) && !empty($coupon_discount) && !empty($coupon_discount_type))
                    {
                        if($coupon_discount_type == '%'){
                    ...
                    if (!in_array($currency, $zero_demial_currencies)) {
                        $amount = $stripe_response->amount / $arm_stripe_amount_multiply;
                    }
        ...
        {
            global $ARMember, $arm_global_settings, $arm_payment_gateways, $arm_debug_payment_log_id, $arm_global_settings;
            $currency = $arm_payment_gateways->arm_get_global_currency();
                ...
                if(!empty($arm_subscribe_plan_details))
                {
                    ...
                    if (!empty($arm_subscribe_plan_amount)) {
                        if (!in_array(strtoupper($currency), $zero_demial_currencies)) {
                            $arm_subscribe_plan_amount = number_format(str_replace(',', '', $arm_subscribe_plan_amount), $arm_currency_decimal, '.', '');
                            $arm_subscribe_plan_amount = $arm_subscribe_plan_amount * $arm_stripe_amount_multiply;
                        }
                        else{
