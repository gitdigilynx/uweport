    ...
    class ARM_shortcodes {
        ...
        function arm_template_shortcode_func($atts, $content, $tag) {
            ...
            $opts = shortcode_atts(array(
                ...
                'search_all_text_label' => __('All','ARMember'),
                    ), $atts, $tag);
            extract($opts);
		...
		if (!empty($spam_protection))
		{
			...
			$form_attr .= ' data-submission-key="' . $captcha_code . '" ';
                ...
                $content .= '</div>';
                $content .= '<form method="post" name="arm_form_ca" id="arm_form' . $formRandomID . '" class="arm_form arm_materialize_form ' . $form_style_class . '" enctype="multipart/form-data" novalidate '.$form_attr.'>';
        ...
        function arm_insert_shortcode_button($content) {
            ...
            if (!in_array(basename( sanitize_text_field( $_SERVER['PHP_SELF'] ) ), $allowed_pages_for_media_button)) {
                return;
            }
        ...
        function arm_insert_shortcode_popup() {
            ...
            if ( !in_array( basename( sanitize_text_field( $_SERVER['PHP_SELF'] ) ), $arm_allowed_pages_for_shortcode_popup ) ) {
                return;
            }
        ...
        function arm_add_tinymce_styles() {
            ...
            if (!in_array(basename( sanitize_text_field( $_SERVER['PHP_SELF'] ) ), $arm_enqueue_shortcode_styles)) {
                return;
            }
            wp_enqueue_script('jquery');
            wp_enqueue_script('arm_bpopup', MEMBERSHIP_URL . '/js/jquery.bpopup.min.js', array('jquery'), MEMBERSHIP_VERSION);
