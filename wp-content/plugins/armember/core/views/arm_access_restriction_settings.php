		...
		<form method="post" action="#" id="arm_access_restriction" class="arm_access_restriction arm_admin_form" onsubmit="return false;">
			...
			<div class="arm_solid_divider"></div>
                        ...
                        { 
                            ...
                            $arm_drip_rules_sql = "SELECT `arm_rule_id` FROM ".$ARMember->tbl_arm_drip_rules." WHERE arm_rule_status='1'";
                            $all_drip_id = $wpdb->get_results($arm_drip_rules_sql);
                        ...
                        ?>
                            ...
                            <div class="page_sub_title" id="arm_global_drip_rules">
                                <?php _e('Restriction Rules for Drip Contents', 'ARMember'); ?>
                                <i class="arm_helptip_icon armfa armfa-question-circle" title="<?php _e("This Drip restriction content will be applied to all drip contents.", 'ARMember'); ?>"></i>
                            </div>
                            <table class="form-table">
                                <tbody>
                                <?php $general_settings['arm_drip_restrict_old_posts'] = !empty($general_settings['arm_drip_restrict_old_posts'])? $general_settings['arm_drip_restrict_old_posts'] : 0;
                                ...
                                <tr class="form-field">
                                    <th class="arm-form-table-label"><?php _e("Allow content prior subscription", 'ARMember'); ?> </th>
                                    <td class="arm-form-table-content">
                                        <div class="armswitch arm_global_setting_switch">
                                            <input type="checkbox" id="arm_drip_restrict_old_posts" value="1" class="armswitch_input" name="arm_general_settings[arm_drip_restrict_old_posts]" <?php checked($general_settings['arm_drip_restrict_old_posts'], '1');?>>
                                            <label for="arm_drip_restrict_old_posts" class="armswitch_label"></label>
                                        </div>
                                        <label for="arm_drip_restrict_old_posts" class="arm_global_setting_switch_label"><?php _e("Allow access to all content publish before subscription/plan purchase.", 'ARMember'); ?></label>
                                    </td>
                                </tr>
                                <tr class="form-field">
                                    <th class="arm-form-table-label"><?php _e("Allow content post subscription",'ARMember');?> </th>
                                    <td class="arm-form-table-content">
                                        <div class="armswitch arm_global_setting_switch">
                                            <input type="checkbox" id="arm_allow_drip_expired_plan" value="1" <?php checked($general_settings['arm_allow_drip_expired_plan'], '1');?> class="armswitch_input" name="arm_general_settings[arm_allow_drip_expired_plan]">
                                            <label for="arm_allow_drip_expired_plan" class="armswitch_label"></label>
                                        </div>
                                        <label for="arm_allow_drip_expired_plan" class="arm_global_setting_switch_label"><?php _e("Allow access to content even after subscription/plan expired which was allowed during subscription.",'ARMember');?></label>
                                    </td>
                                </tr>
                                <tr class="form-field arm_sync_drip_row <?php echo (!empty($general_settings['arm_allow_drip_expired_plan']) && empty($general_settings['arm_allow_drip_expired_plan_is_sync'])) ? '' : 'hidden_section'; ?>" >
                                    <th class="arm-form-table-label"></th>
                                    <td class="arm-form-table-content arm_drip_rule_sync_btn_div">
                                        <input type="hidden" value="<?php echo $drip_ids;?>" class="arm_drip_ids_sync">
                                        <input type="hidden" value="<?php echo count($arm_drip_array);?>" class="arm_drip_ids_sync_total">
                                        <input type="hidden" value="0" class="arm_drip_ids_sync_process">
                                        <input type="hidden" value="<?php echo $general_settings['arm_allow_drip_expired_plan_is_sync'];?>" name="arm_general_settings[arm_allow_drip_expired_plan_is_sync]" id="arm_allow_drip_expired_plan_is_sync">

                                        <span class="arm_warning_text arm_margin_0 arm_margin_left_10 arm_margin_bottom_10">
                                            <em><?php _e('You need to sync dripped content once after enabling "Allow content post subscription" option. Please click the "Sync Dripped Content" button.','ARMember');?></em>
                                        </span>
                                        <img src="<?php echo MEMBERSHIP_IMAGES_URL.'/arm_loader.gif' ?>" id="arm_loader_img_sync" class="arm_submit_btn_loader" style="display:none;" width="24" height="24">&nbsp;<button class="arm_save_btn arm_drip_rule_sync_btn arm_margin_left_5" data-total-users="3065" type="button" id="arm_drip_rule_sync_btn" name="arm_drip_rule_sync_btn" onClick="showConfirmBoxCallback_sync();"><?php _e('Sync Dripped Content','ARMember');?></button>
                                        ...
                                        <div class='arm_confirm_box' id='arm_confirm_box_arm_drip_sync'>
                                            <div class='arm_confirm_box_body'>
                                                ...
                                                <div class='arm_confirm_box_text'><?php _e('Are you sure you want to sync dripped content data?','ARMember')?> </div>
                                                <div class='arm_confirm_box_btn_container'>
                                                    <button type='button' class='arm_confirm_box_btn armok arm_start_drip_data' data-item_id='arm_start_drip_sync' data-type='arm_start_drip_sync'><?php _e('Yes', 'ARMember') ?></button>
                                                    <button type='button' class='arm_confirm_box_btn armcancel' onclick='hideConfirmBoxCallback_sync();'><?php _e('No', 'ARMember') ?></button>
                                                </div>
                                            </div>
                                        </div>
                                    </td>
